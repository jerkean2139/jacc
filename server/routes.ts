...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/training/test', isAuthenticated, async (req, res) => {
    try {
      ...
      // Simple test response for training mode
      const testResponse = {
        message: `Training mode response for: "${query}"\n\nThis is a test response that would normally be generated by the AI system. In production, this would include:\n\n• Document search results\n• Knowledge base queries\n• Contextual merchant services information\n• Structured recommendations\n\nUse this interface to test queries and provide feedback for improving AI responses.`,
        sources: [
          {
            name: "Training Knowledge Base",
            url: "/training",
            relevanceScore: 0.95,
            snippet: "Sample training data for merchant services",
            type: "knowledge_base"
          }
        ],
        reasoning: "This is a test response generated in training mode for admin evaluation and feedback."
      };

      res.json({
        response: testResponse.message,
        sources: testResponse.sources,
