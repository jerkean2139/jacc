...
Would you like me to create a detailed proposal for this merchant?`,
  ...
  // Chat Review Center endpoints
  app.get('/api/admin/chat-reviews', async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db.ts');
      const { chats, users, messages, chatReviews } = await import('../shared/schema.ts');
  ...
  app.get('/api/admin/chat-reviews/:chatId', async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db.ts');
      const { chats, users, messages, chatReviews, messageCorrections } = await import('../shared/schema.ts');
      const { eq } = await import('drizzle-orm');
      ...
      const messageCorrectionsList = await db
        .select()
      ...
      res.json({
        ...
        corrections: messageCorrectionsList,
      });
    } catch (error) {
  ...
  app.post('/api/admin/chat-reviews/:chatId/correct-message', async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db.ts');
      const { messages, messageCorrections } = await import('../shared/schema.ts');
      const { eq } = await import('drizzle-orm');
      const { unifiedLearningSystem } = await import('./unified-learning-system');
      
      const { chatId } = req.params;
      const { messageId, originalResponse, correctedResponse, correctionReason, improvementType } = req.body;
      ...
      // Save the message correction
      await db.insert(messageCorrections).values({
        messageId,
        chatId,
        originalResponse,
        correctedResponse,
        correctionReason,
        correctedBy: userId,
        improvementType: improvementType || 'accuracy',
      });

      // Update the original message with the correction
      await db
        .update(messages)
        .set({ content: correctedResponse })
        .where(eq(messages.id, messageId));

      // Add to unified learning system
      await unifiedLearningSystem.addTrainingInteraction({
        source: 'admin_correction',
        query: 'Message correction',
        response: originalResponse,
        correctedResponse,
        wasCorrect: false,
        metadata: {
      ...
      console.error('Error saving message correction:', error);
  ...
  app.post('/api/admin/chat-reviews/:chatId/review', async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db.ts');
      const { messages, chatReviews, messageCorrections } = await import('../shared/schema.ts');
  ...
  app.get('/api/admin/chat-reviews/stats', async (req: Request, res: Response) => {
    try {
      const { db } = await import('./db.ts');
      const { chats, chatReviews, messageCorrections } = await import('../shared/schema.ts');
